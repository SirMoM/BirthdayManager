...
public class UpdateBirthdaysThisMonthTask extends PersonTasks<List<PersonsInAMonthWeek>> {
  ...
  private static final Logger LOG =
      LogManager.getLogger(UpdateBirthdaysThisMonthTask.class.getName());
  ...
  protected List<PersonsInAMonthWeek> call() throws Exception {
    ...
    personDB.sort(
        Comparator.comparing(
            person ->
                BirthdayUtils.getBirthdayInYear(person.getBirthday(), this.targetMonth.getYear())));
    ...
    LOG.debug(
        "Gathered all Persons this month({})! Count: {}",
        this.targetMonth,
        birthdaysThisMonth.size());
