...
public class CheckMissedBirthdays extends Task<List<Person>> {
  ...
  protected List<Person> call() throws Exception {
    ...
    LocalDate lastVisit =
        LocalDate.parse(PropertyManager.getProperty(PropertyFields.LAST_VISIT), dateTimeFormatter);
    PropertyManager.getInstance()
        .getProperties()
        .setProperty(PropertyFields.LAST_VISIT, LocalDate.now().format(dateTimeFormatter));
  ...
  private List<Person> getBrithdaysSince(LocalDate lastVisit, LocalDate now) {
    ...
    for (Person person : personDB) {
      if (person.getBirthday().withYear(now.getYear()).isAfter(lastVisit)
          && person.getBirthday().withYear(now.getYear()).isBefore(now)) {
