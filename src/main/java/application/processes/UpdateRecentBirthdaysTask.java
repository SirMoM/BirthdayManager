...
public class UpdateRecentBirthdaysTask extends PersonTasks<List<Person>> {
  ...
  public UpdateRecentBirthdaysTask() {
    String showBirthdaysCountProperty =
        PropertyManager.getProperty(PropertyFields.SHOW_BIRTHDAYS_COUNT);
  ...
  protected List<Person> call() throws Exception {
    ...
    for (; i < NEXT_BIRTHDAYS_COUNT; i++) {
      try {
        ...
        LOG.debug(
            "Probably not enough Persons to gather the birthdays to fill the recent List",
    ...
    for (; j < NEXT_BIRTHDAYS_COUNT - i; j++) {
      try {
        ...
        LOG.debug(
            "Probably not enough Persons to gather the 10 birthdays for recent",
            indexOutOfBoundsException);
        break;
      }
    }
