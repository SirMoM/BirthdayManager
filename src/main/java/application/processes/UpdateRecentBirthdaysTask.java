...
public class UpdateRecentBirthdaysTask extends Task<List<Person>> {
  ...
  public UpdateRecentBirthdaysTask() {
    ...
    NEXT_BIRTHDAYS_COUNT =
        Integer.parseInt(PropertyManager.getProperty(PropertyFields.SHOW_BIRTHDAYS_COUNT));
  ...
  protected List<Person> call() throws Exception {
    ...
    for (; i < NEXT_BIRTHDAYS_COUNT; i++) {
      try {
        ...
        LOG.debug(
            "Probably not enought Persons to geather the 10 birthdays for recent",
