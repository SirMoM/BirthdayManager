...
public class MainController {
  ...
  final EventHandler<ActionEvent> openPreferencesHander =
      event -> {
  ...
  final EventHandler<ActionEvent> exportToFileHandler =
      event -> {
        ...
        fileChooser
            .getExtensionFilters()
            .addAll(
                new ExtensionFilter("CSV Files", CSV_FILE_EXTENSION),
  ...
  final EventHandler closeAppHandler =
      event -> {
              ...
              .ifPresent(
                  type -> {
                    if (type.getButtonData() == ButtonType.YES.getButtonData()
                      ...
                      try {
                        fileChooser.setInitialDirectory(
                            new File(PropertyManager.getProperty(PropertyFields.LAST_OPEND))
                                .getParentFile());
                      } catch (final NullPointerException nullPointerException) {
                        fileChooser.setInitialDirectory(new File(System.getProperty(USER_HOME)));
                      }
        ...
        try {
          ...
          exportToCalenderTask.setOnSucceeded(
              x -> {
                ...
                alert.setHeaderText(
                    "All bithdays have been exported to " + saveFile.getAbsolutePath() + "!");
                alert.showAndWait();
              });
  ...
  public MainController(final Stage stage) {
    ...
    this.stage
        .getIcons()
        .add(new Image(this.getClass().getResourceAsStream("/img/icons8-birthday-50.png")));
  ...
  private void openFile(final File selectedFile) throws IOException {
    ...
    PropertyManager.getInstance()
        .getProperties()
        .setProperty(PropertyFields.LAST_OPEND, getSessionInfos().getSaveFile().getAbsolutePath());
    try {
