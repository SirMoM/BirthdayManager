...
public class BirthdaysOverviewController extends Controller {
        ...
        public ListCell<Person> call(final ListView<Person> param) {
          ...
          return new ListCell<Person>() {
            ...
            protected void updateItem(final Person item, final boolean empty) {
              ...
              if (!empty && item != null) {
                if (item.getBirthday()
                    .isEqual(LocalDate.now().withYear(item.getBirthday().getYear()))) {
                  this.styleProperty()
                      .bind(
                        ...
                        .withYear(item.getBirthday().getYear())
                        .isBefore(item.getBirthday())
                    && item.getBirthday()
                        .isBefore(
                            LocalDate.now().withYear(item.getBirthday().getYear()).plusDays(7))) {
  ...
  private final EventHandler<ActionEvent> importBirthdaysHandler =
      event -> {
        ...
        BirthdaysOverviewController.this.LOG.debug(
            "Importing file:" + selectedFile.getAbsolutePath());
  ...
  private void bindComponents() {
    ...
    this.preferences_MenuItem.addEventHandler(
        ActionEvent.ANY, this.getMainController().openPreferencesHander);
    ...
    this.refresh_MenuItem.setOnAction(
        actionEvent -> {
          this.refreshBirthdayTableView();
        });

    this.debug.setOnAction(
        actionEvent -> {
  ...
  private void createRecentFilesMenueItems() {
    ...
    this.recentFiles_MenuItem
        .textProperty()
        .bind(this.getMainController().getSessionInfos().getRecentFileName());
