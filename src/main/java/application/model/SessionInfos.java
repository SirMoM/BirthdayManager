...
public class SessionInfos {
  ...
  public SessionInfos(MainController mainController) {
    ...
    try {
      this.getRecentFileName()
          .set(new File(PropertyManager.getProperty(PropertyFields.LAST_OPEND)).getName());
  ...
  public void updateSubLists() {
    ...
    if (this.mainController.getActiveController() instanceof BirthdaysOverviewController) {
      progressBar =
          ((BirthdaysOverviewController) this.mainController.getActiveController())
              .getProgressbar();
    } else {
      progressBar = null;
    }
            ...
            for (Person person : value) {
              ...
              long days =
                  ChronoUnit.DAYS.between(
                      person.getBirthday().withYear(LocalDate.now().getYear()), LocalDate.now());
