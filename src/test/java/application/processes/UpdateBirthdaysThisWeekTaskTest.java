...
class UpdateBirthdaysThisWeekTaskTest {
    ...
    void call_ReturnsOnlyBirthdaysFromTheCurrentWeek() throws Exception {
        ...
        PersonManager.getInstance().setPersonDB(new ArrayList<>(List.of(
                nextWeekBirthday,
                sundayBirthday,
                mondayBirthday,
                wednesdayBirthday
    ...
    void call_CreatesAnotherWeekRowWhenTwoBirthdaysShareTheSameWeekday() throws Exception {
        ...
        assertThat(result.stream()
                .map(PersonsInAWeek::getMondayPerson)
                .toList())
    ...
    private Person personForDate(String name, LocalDate date) {
        return new Person(name, "Mustermann", null, LocalDate.of(1990, date.getMonth(), date.getDayOfMonth()));
    }
}
